...
You build a nonlinear **expression graph** from the `sd_*` atom
constructors, assemble it into a **problem**, and evaluate — at any
primal point — the objective value, its gradient, the sparse constraint
Jacobian (COO), and the lower-triangular Lagrangian Hessian (COO).

This is a low-level backend. The intended user is a higher-level
modelling layer such as [CVXR](https://github.com/cvxgrp/CVXR), not
someone writing models by hand.
...
The SparseDiffEngine C sources are bundled and built with R’s own
toolchain and BLAS; there is nothing external to install.
...
See
[`vignette("sparsediff")`](https://bnaras.github.io/sparsediff/articles/sparsediff.md)
for the full walk-through, including parameters and fast re-evaluation.

## License

Apache License 2.0. The bundled SparseDiffEngine is by Daniel Cederberg
and William Zijie Zhang.
